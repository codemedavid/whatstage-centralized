 ...
 * 
 * ⚠️ DISABLED: Auto follow-ups are now disabled.
 * Follow-ups should only happen via published workflows.
 * To re-enable, remove the early return below.
 */
export async function GET(req: Request) {
    // Auto follow-ups disabled - use workflows instead
    console.log('[FollowUpCron] Auto follow-ups disabled. Use workflows for follow-ups.');
    return NextResponse.json({
        processed: 0,
        message: 'Auto follow-ups disabled. Use workflows instead.'
    });
